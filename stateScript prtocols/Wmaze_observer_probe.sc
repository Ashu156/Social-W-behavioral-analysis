...
%   - Motorized barriers go up for both correct and incorrect choices.
%   - Upon correct choice, reward is delivered and the corresponding barrier lifts.
%   - Upon incorrect choice, no reward is delivered but the barrier still lifts.
...
%
% Output Ports:
%   Reward Pumps: rewardPump1 (port 1), rewardPump2 (port 5), rewardPump3 (port 3)
%   Barriers: barrier1 (port 18), barrier2 (port 17), barrier3 (port 19)
%
% Notes:
...
int rewardPump1 = 8      % Left well reward pump
int rewardPump2 = 7      % Center well reward pump
int rewardPump3 = 5      % Right well reward pump

int barrier1_left   = 21  % Left arm barrier port
int barrier2_center = 20  % Center arm barrier port
int barrier3_right  = 22  % Right arm barrier port
...
callback portin[13] up  % Left well
    ...
    if lastWell == 0 do
        ...
        trigger(4)  % Raise barrier
        trigger(5)  % Lower after delay
    else if lastWell != 1 do
        disp('Left well not rewarded - incorrect choice')
        trigger(4)  % Raise barrier
        trigger(5)  % Lower after delay
    else do
        disp('Left well not rewarded - repeated poke')
    end
end;
...
callback portin[4] up  % Center well
    ...
    if lastWell == 0 do
        ...
        trigger(4)  % Raise barrier
        trigger(5)  % Lower after delay
    else if lastWell != 2 do
        disp('Center well not rewarded - incorrect choice')
        trigger(4)  % Raise barrier
        trigger(5)  % Lower after delay
    else do
        disp('Center well not rewarded - repeated poke')
    end
end;
...
callback portin[8] up  % Right well
    ...
    if lastWell == 0 do
        ...
        trigger(4)  % Raise barrier
        trigger(5)  % Lower after delay
    else if lastWell != 3 do
        disp('Right well not rewarded - incorrect choice')
        trigger(4)  % Raise barrier
        trigger(5)  % Lower after delay
    else do
        disp('Right well not rewarded - repeated poke')
    end
end;
